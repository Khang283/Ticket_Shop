...
const EditTicketForm = ({ ticket, onClose }) => {
    ...
    const [selectedTicketType, setSelectedTicketType] = useState(ticket.ticketTypeId);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:3000/api/v1/tickets/${ticket.id}`, {
                name: ticketName,
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-60">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md text-left">
                <h1 className="text-center">Cập nhật vé</h1>
